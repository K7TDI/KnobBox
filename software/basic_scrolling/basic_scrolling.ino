 ...
 * 
 * This sketch will display the global variable 'leftposition' in the top left of the screen,
 * the last read position of the encoder on the bottom left of the screen, the difference between the two
 * in the upper right, and a timer in the lower right.
 * Every 100 ms, the variable 'difference' is updated to the encoder value minus the global variable value,
 * and the global value is set to the encoder value.  This allows for a small buffering of the change before spitting
 * out commands.  At this interval the mouse scroll wheel command is sent with the negative value of "difference".
 * Every 5 seconds, the encoder register and the leftposition are reset to 5000.
 * 
